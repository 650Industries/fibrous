// CodeClimate: don't ignore CoffeeScript 1.9.0
(function() {
  var Fiber, Future, base, co, defineMemoizedPerInstanceProperty, fibrous, functionWithFiberReturningFuture, futureize, isGeneratorFunction, proxyAll, proxyBuilder, synchronize, _i, _len, _ref,
    __slice = [].slice;
  ...
  futureize = function(asyncFn) {
    return function() {
      ...
      if (isGeneratorFunction(asyncFn)) {
        co(asyncFn.bind.apply(asyncFn, [fnThis].concat(__slice.call(args)))).then(function(result) {
          var resolve;
